...
class Order {
    total = 0
    order = []
    constructor(status_order, order_date, customer_name, customer_address, customer_phone, platform) {
...
function addToOrder(id, name, price) {
    currentOrder.addProduct({ id, name, price, quantity: 1 });
    const productCart = document.getElementById("search_order")

    currentOrder.order.forEach(({ id, name, price, quantity }) => {
        console.log({ id, name, price, quantity })
        let temp = document.createElement("div")
        temp.setAttribute("class", "d-flex justify-content-between")
        temp.setAttribute("style", "width: 502px; background-color: white; text-align: center")

        temp.innerHTML = `<p style="width: 110px;">${id}</p>
        ...
        buttonPlus.onclick = () => {
            currentOrder.plusQuantityProduct(id, 1)
        }

        buttonMinus.onclick = () => {
            currentOrder.minusQuantityProduct(id, 1)
        }
...
searchInput.oninput = (e) => {
    fetch(`https://9bf9-14-161-27-27.ngrok-free.app/api/products?product_name=${e.target.value}&platform=Shopee`)
        ...
        .then((res) => {
            ...
            if (res?.data.length > 0) {
                ...
                res.data.forEach(({ id_product, product_name, price }) => {
                    let temp = document.createElement("div")
                    temp.setAttribute("class", "d-flex justify-content-between productDiv")
                    temp.setAttribute("style", "width: 502px; background-color: white; text-align:center")
                    temp.innerHTML =
                        `<p class="id">${id_product}</p>
