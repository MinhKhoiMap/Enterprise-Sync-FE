...
function validateAndContinue(nextPage) {
  ...
  if (isValid) {
    ...
    activateButton(
      document.querySelector('.circle[data-frame="' + nextPage + '"]')
    );
    showCurrentPage();
  } else {
...
function validateAndContinue2(nextPage) {
  ...
  var shopeePassword1 = document.getElementById(
    "exampleFormControlInput6"
  ).value;
  var shopeeEmail2 = document.getElementById("exampleFormControlInput7").value;
  var shopeePassword2 = document.getElementById(
    "exampleFormControlInput8"
  ).value;
  ...
  if (shopeeEmail2.trim() === "" && shopeePassword2.trim() === "") {
    ...
    if (!isValidPassword(shopeePassword1)) {
      document.getElementById("shopeePasswordError1").innerHTML =
        "Mật khẩu Shopee phải có ít nhất 6 ký tự.";
      isValid = false;
    } else {
      document.getElementById("shopeePasswordError1").innerHTML = "";
    }
...
function submitAccount() {
  ...
  axios
    .post(`${DOMAIN_SERVICE}/api/users`, request)
    .then((response) => {
